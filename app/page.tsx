...
export default function Home() {
  ...
  const handleStartGame = (e: React.FormEvent) => {
    ...
    if (isNewUser === false && username.toLowerCase() === "عبدالرحمن" && pin !== "1234") {
    ...
    const selectedItems = currentItems.slice(0, Math.min(pairCount, currentItems.length));
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center min-h-screen bg-linear-to-b from-amber-50 to-orange-100 p-4 select-none text-right" dir="rtl">
          ...
          {isNewUser === null ? (
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <label className="font-semibold text-slate-600">اسم المستخدم الموحد:</label>
                <input
                  type="text"
                  value={username}
              ...
              {authError && <p className="text-rose-500 text-sm font-semibold">{authError}</p>}
              <button
                type="button"
            ...
            <form onSubmit={handleStartGame} className="flex flex-col gap-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
                <p className="text-sm text-slate-500 font-medium">مرحباً بك، <span className="text-amber-600 font-bold">{username}</span></p>
                <p className="text-xs text-emerald-600 font-bold mt-1">
                  {isNewUser ? "✨ حساب جديد! اختر رمز سري لحفظ نقاطك." : "🔒 حساب محجوز! أدخل الرمز السري لتأكيد هويتك."}
                </p>
              </div>

              <div className="flex flex-col gap-2">
                <label className="font-semibold text-slate-600">الرمز السري (4 أرقام):</label>
                <input
                  type="password"
                  value={pin}
              ...
              <div className="flex flex-col gap-2">
                <label className="font-semibold text-slate-600">تصنيف الكروت (ثيم اللعبة):</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="font-semibold text-slate-600">مستوى الصعوبة:</label>
                <div className="grid grid-cols-3 gap-2">
                  {(["easy", "medium", "hard"] as const).map((level) => (
                    ...
                    >
                      {level === "easy" ? "سهل (12)" : level === "medium" ? "متوسط (16)" : "صعب (24) 🔥"}
                    </button>
                  ))}
                </div>
              </div>
      ...
      {screen === "game" && (
        <div className="flex flex-col items-center animate-fade-in">
          ...
          <div className="flex items-center gap-6 mb-6 bg-white px-6 py-3 rounded-xl shadow-md border border-amber-100">
            <div className="text-slate-700">الوقت: <span className="font-bold text-amber-600">{time}</span> ثانية</div>
            <div className="text-slate-700">المحاولات: <span className="font-bold text-amber-600">{moves}</span></div>
            <button
              onClick={resetMenu}
          ...
          <div className={`grid ${getGridCols()} gap-3 sm:gap-4 bg-white p-6 rounded-2xl shadow-xl border border-amber-100 max-w-full justify-center justify-items-center`}>
            {cards.map((card) => {
              const isIconComponent = typeof card.value === "function";
              const IconComponent = isIconComponent
                ? (card.value as unknown as React.ComponentType<{ className?: string }>)
                : null;
              ...
              return (
                ...
                >
                  {card.isFlipped || card.isMatched ? (
                    IconComponent ? (
                      <IconComponent className={`w-8 h-8 sm:w-12 sm:h-12 ${card.color || 'text-slate-700'} animate-scale-up`} />
                    ) : (
